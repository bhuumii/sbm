...
export const AboutPageClient = ({ data }: { data: AboutPageData }) => {
  ...
  const portableTextComponents = {
    block: {
      ...
      h3: ({children}: any) => (
        <h3 className={`text-lg md:text-xl font-semibold mb-3 mt-6 ${
        ...
        if (startsWithValue) {
          return (
            <div className="flex items-start mb-4">
              ...
              <p className={`text-base md:text-lg leading-7 flex-1 ${
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${
        ...
        <div className="text-center">
          ...
          <p className={isDark ? "text-gray-300" : "text-gray-600"}>Content for this page has not been added yet.</p>
        </div>
      </div>
    );
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {data.teamMembers.map((member, index) => (
                  <ScrollAnimationWrapper 
                    ...
                    className="group"
                  >
                    <div className={`rounded-xl shadow-lg hover:shadow-xl overflow-hidden text-center transform hover:-translate-y-2 transition-all duration-300 border ${
                      isDark 
